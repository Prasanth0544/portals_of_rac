...
function App({ initialPage }: AppProps): React.ReactElement {
  ...
  // Timer display state â€” polls backend engine for remaining time (display only)
  ...
  useEffect(() => {
    const restoreState = async () => {
      ...
      try {
        ...
        // Backend unreachable â€” use saved state as-is
  ...
  // Local 1-second countdown (visual only â€” backend engine does the real work)
  ...
  const autoInitializeFromBackend = async (): Promise<void> => {
    ...
    try {
      console.log('ðŸ”§ Attempting auto-initialization from backend config...');
      ...
      if (config.success && config.data.isConfigured) {
        console.log('âœ… Backend is configured, auto-initializing train...');
        ...
        if (response.success) {
          console.log('âœ… Train auto-initialized successfully!');
          await loadTrainState();
          // Only go to home if currently on config â€” don't override other pages
          setCurrentPage(prev => prev === 'config' ? 'home' : prev);
        } else {
          console.warn('âš ï¸ Auto-initialization failed:', response.error);
          // Stay on current page â€” don't redirect to config
        }
      } else {
        console.log('â„¹ï¸ Backend not configured â€” user will need to use config page');
      }
    } catch (error: any) {
      console.warn('âš ï¸ Auto-initialization error:', error.message);
      // Stay on current page â€” don't redirect to config
    }
  };
  ...
  const handleMarkNoShow = async (pnr: string): Promise<void> => {
    try {
      ...
      if (response.success) {
        ...
        alert(`âŒ Error: ${response.error}`);
      }
    } catch (err: any) {
      alert(`âŒ Error: ${err.message || 'Failed to mark no-show'}`);
    } finally {
      setLoading(false);
    }
  };
