  ...
  try {
    ...
    try {
      if (mongoUri) {
        ...
        console.warn(
          "⚠️ No MONGODB_URI in .env — waiting for runtime configuration via /api/config/setup",
        );
      }
      ...
      try {
        ...
        if (racDb) {
          const stationReallocations = racDb.collection(
            COLLECTIONS.STATION_REALLOCATIONS,
          );
          const reallocResult = await stationReallocations.deleteMany({});
          if (reallocResult.deletedCount > 0) {
            console.log(
              `🗑️ Server start: Cleared ${reallocResult.deletedCount} old reallocations`,
            );
          }

          const upgradeNotifications = racDb.collection(
            COLLECTIONS.UPGRADE_NOTIFICATIONS,
          );
          const notifResult = await upgradeNotifications.deleteMany({});
          if (notifResult.deletedCount > 0) {
            console.log(
              `🗑️ Server start: Cleared ${notifResult.deletedCount} old notifications`,
            );
          }
        ...
        console.warn(
          "⚠️ Could not clear old data on startup:",
          cleanupErr.message,
        );
      }
    } catch (error) {
    ...
    httpServer.listen(PORT, () => {
      ...
      console.log(
        `   Mode:            Multi-Train (per-train config from Trains_Details)`,
      );
      console.log("");
      console.log(
        `📡 WebSocket Server: Ready (${wsManager.getClientCount()} clients)`,
      );
      console.log("");
